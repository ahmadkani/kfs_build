...
export class VFSutils {
  ...
  async initialize() {
    ...
    try {
      ...
      if (this.fetchInfo.corsProxy) {
        await this.workerThread.setCorsProxy(this.fetchInfo.corsProxy);
      }
  ...
  async fetchFromGit() {
    try {
      ...
      await this.workerThread.doCloneAndStuff({ url });
  ...
  async generateFsTable() {
    try {
      ...
      const fileList = await this.workerThread.listFilesDot(1);
  ...
  async commitStagedChanges( message ) {
    try {
      ...
      return await this.workerThread.commitStagedChanges(message);
    } catch (error) {
   ...
   */
      ...
      try {
        ...
        switch (strategy) {
          case 'local-ahead':
            ...
            await this.workerThread.push({ url, ref: 'main' });
            break;
          case 'other-cases':
    ...
    async getSyncStatus(_url = null, ref = 'main') {
      try {
        ...
        const localHead = await this.workerThread.getLastLocalCommit(ref);
        const getRemoteHead = await this.workerThread.getLatestRemoteCommit({ url, ref });
        ...
        const mergeBase = await this.workerThread.findMergeBase([localHead, remoteHead]);
